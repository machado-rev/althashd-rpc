...
const debug = createDebug('althashd-rpc');

export default class AlthashRpc {
  constructor(readonly url: string, readonly options: CreateAlthashRpcOptions = {}) {
    this.url = url;
    ...
    const attempt: (attemptN?: number) => any = async (attemptN = 1) => {
      ...
      try {
        ...
        if (attemptN === maxAttempts) {
          throw new AlthashRpcError(error, executed, getErrrorData());
        }
